        ...
        class DataFlowAnalysis {
            ...
            public:
                ...
                void runWorklistAlgorithm(Function * func) {
                    ...
                    for (auto itr = IndexToInstr.begin(); itr != IndexToInstr.end(); ++itr) {
                        worklist.push_back(itr->first);
                    }
                    ...
                    while (worklist.size() != 0) {
                        ...
                        std::vector<Info *> info_outs(outgoing_edges.size());
                        for (int i = 0; i < info_outs.size(); ++i) {
                            info_outs[i] = new Info();
                        }
                        flowfunction(instr, incoming_edges, outgoing_edges, info_outs);

                        for (int i = 0; i < outgoing_edges.size(); ++i) {
                            Edge out_edge = std::make_pair(index, outgoing_edges[i]);
                            Info *new_info = Info::join(EdgeToInfo[out_edge], info_outs[i], nullptr);
                            if (!Info::equals(new_info, EdgeToInfo[out_edge])) {
                                EdgeToInfo[out_edge] = new_info;
                                worklist.push_back(outgoing_edges[i]);
                            } else {
                                delete new_info;
                            }
                        }
